...
$baseURL = "http://tiwar.ru";
$debug = 0;
var_dump($argv);
$username = $argv[1];
$password = $argv[2];
echo "Clear cookie\n";
@unlink(__DIR__."/$username.txt");
...
function postUrl($url,$post, $ref) {
    ...
    curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
    curl_setopt($ch, CURLOPT_USERAGENT,"Mozilla/5.0 (Windows NT 6.3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.116 Safari/537.36");
    curl_setopt($ch, CURLOPT_COOKIEFILE, __DIR__."/$username.txt");
    curl_setopt($ch, CURLOPT_COOKIEJAR, __DIR__."/$username.txt");
